const endpoints = {
  ...
  villages: (state, district, taluka) => `/api/villages/${state}/${district}/${taluka}`,
  distance: (srcFull, dstFull) => `/api/distance?src=${srcFull}&dest=${dstFull}`,
  location: (full) => `/api/location/${full}`
};
...
async function loadVillages(side, stateCode, districtCode, talukaCode){
  ...
  const villages = await fetchJson(endpoints.villages(stateCode, districtCode, talukaCode));
  villages.forEach(v => {
    const opt = document.createElement('option');
    opt.value = v.code;
...
async function getDistanceAndShow(){
  ...
  if(!srcFull || !dstFull){
    alert('Please select at least state and district for both source and destination');
    return;
  }
...
async function openRouteOnMaps(){
  ...
  const srcFull = buildFullCode(s_state, s_district, s_taluka, s_village);
  const dstFull = buildFullCode(d_state, d_district, d_taluka, d_village);
  if(!srcFull || !dstFull){ alert('Select state+district for both sides'); return; }

  try{
    const sLoc = await fetchJson(endpoints.location(srcFull));
    const dLoc = await fetchJson(endpoints.location(dstFull));
    if(!sLoc || !dLoc || !sLoc.latitude || !dLoc.latitude){ alert('Latitude/Longitude not found for one or both locations'); return; }
...
window.addEventListener('load', async ()=>{
  ...
  $('src_state').addEventListener('change', async (e)=>{ await loadDistricts('src', e.target.value); });
  $('src_district').addEventListener('change', async (e)=>{ const s = $('src_state').value; await loadTalukas('src', s, e.target.value); });
  $('src_taluka').addEventListener('change', async (e)=>{ const s = $('src_state').value; const d = $('src_district').value; await loadVillages('src', s, d, e.target.value); });

  // Destination side
  $('dst_state').addEventListener('change', async (e)=>{ await loadDistricts('dst', e.target.value); });
  $('dst_district').addEventListener('change', async (e)=>{ const s = $('dst_state').value; await loadTalukas('dst', s, e.target.value); });
  $('dst_taluka').addEventListener('change', async (e)=>{ const s = $('dst_state').value; const d = $('dst_district').value; await loadVillages('dst', s, d, e.target.value); });
